...
def fetch_bike_data(api_url):
    ...
    if response.status_code == 200:
        ...
        print("Fetched bike data successfully.")
        ...
        for station in bike_data:
            ...
            if not os.path.exists(csv_file_path):
                ...
                print(f"Saved new bike data for station: {station_name}")
            else:
                ...
                if station["timestamp"] not in original_df["timestamp"].values:
                    ...
                    print(f"Updated bike data for station: {station_name}")
    else:
        ...
        if "hourly" in weather_data and "time" in weather_data["hourly"]:
            ...
            if not weather_df.empty:
                ...
                if not os.path.exists(weather_file_path):
                    ...
                    print(f"Saved new weather data for station: {station_name}")
                else:
                    ...
                    print(f"Updated weather data for station: {station_name}")
            else:
                print(f"No valid weather data to save for {station_name}")
        else:
            print(f"No valid hourly data for {station_name}. Response:")
            print(weather_data)
    else:
        print(f"Failed to fetch weather data for {station_name}. Status code: {response.status_code}")
